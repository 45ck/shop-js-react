...
import React, { ComponentProps, MutableRefObject, useEffect, useRef, useState } from 'react'
...
export default function RowItem(prop: {ownerItem: OwnerItem, picture?: Picture, className?: string}) {
  return (
    <>
        <div className={`border-solid border-t-gray-800 border-2 cursor-pointer ${prop.className}`} title={`View ${prop.ownerItem.item.name}`} onClick={() => Router.push(`/items/${prop.ownerItem.item.id}`)}>
